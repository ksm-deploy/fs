...
if authentication_status:
    ...
    if int(sc_t) < 1500:
        ...
        # st.text(매출)
            ...
            x=alt.X('중분류', sort=None, title=""), y=alt.Y(f'{기준년도}_N',axis=alt.Axis(labels=False)),  color=alt.Color(f'{기준년도}_N',legend=None))
        # ,trendline="ols"
            ...
            x=alt.X('중분류', sort=None, title=""),  y=alt.Y(f'{기준년도}_N',axis=alt.Axis(labels=False), title=""),  detail=f'{기준년도}_N', text=alt.Text(f'{기준년도}_N:Q'))
    ...
    # chart.update_layout(font=dict(size=14))
        ...
        with tab2:
            ...
            st.info("사업별손익")
            사업구분 = st.selectbox("사업구분선택",["전체","공연","전시"], index= 0)
        ...
        with tab3:
            ...
            def m_chart(key1, 대상항목):    
                ...
                x=alt.X('일자', sort=None, title=""),  y=alt.Y('금액',axis=alt.Axis(labels=False), title=""),  detail='금액', text=alt.Text('금액:Q',format=",.0f"))
                # st.altair_chart(chart+text, use_container_width=True)
            ...
            st.info("사업별손익")
            사업구분2 = st.selectbox("사업구분선택 ",("전체","공연","전시"), index= 0)
            대상항목 = st.selectbox("항목선택",["매출","사업비","인건비","일반관리비","건물관리비","지급임차료","영업이익"], index= 0)
            if 사업구분2 == "전체":
                df_손익_전체_누계 = templit("월별손익", df_all, df_tem , cost_SORT1, cost_SORT2, cond_전체)
