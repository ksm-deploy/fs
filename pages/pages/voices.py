...
search_word = st.text_input("키워드 입력 : 키워드 입력 후 Enter","롯데뮤지엄")
...
def make_clickable(row):
    text = row['text']
    link = row['rinks']
    val = f"<a target='_blank' href='{link}'>{text}</a>"
    return val

df_news_b['text'] = df_news_b.apply(make_clickable, axis=1)

df_news_b = df_news_b.drop(['rinks'], axis=1)
# st.dataframe(df_news_b, use_container_width=True, hide_index=True)
st.write(df_news_b.to_html(escape=False, index=False), unsafe_allow_html=True)
