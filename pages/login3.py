...
if authentication_status:
    ...
    if int(sc_t) < 1500:
        ...
        with col2:
            st.text(f"지출 : {지출}")
        # st.text(매출)
        chart = alt.Chart(df_tem, title=f'자금수지효과 : {손익효과}억').properties(height=500).mark_bar().encode(
	    x=alt.X('중분류', sort=None,axis=alt.Axis(labels=False)), y=alt.Y('2024_N'),  color='2024_N')
    # ,trendline="ols"
	    ...
	    x=alt.X('중분류', sort=None,axis=alt.Axis(labels=False)),  y=alt.Y('2024_N'),  detail='2024_N', text=alt.Text('2024_N:Q'))
