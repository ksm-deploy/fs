...
def get_posts(query):
    ...
    for n in bloggers:
        ...
        # df_test.loc[idx] = [bloger,0]
        # idx += 1
        # st.text(texts_l)
    df_test = pd.DataFrame(data=texts_l,columns=['text'], index=None)

    # df_test = pd.DataFrame({"test":texts_l})
    # return df_test
    return st.dataframe(df_test, hide_index=True, use_container_width=True)
...
#     search_url = urllib.request.urlopen(url).read()
#     soup = BeautifulSoup(search_url, 'html.parser')
#     posts = soup.find_all('ul',{'class':'lst_view _fe_view_infinite_scroll_append_target'})

#     for post in posts:
#         title = post.find('li',{'class':'bx'}).find('div',{'class':'view_wrap'}).find('div',{'class':'user_box_inner'}).get_text()
